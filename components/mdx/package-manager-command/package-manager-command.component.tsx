...
const packageManagerSchema = z
  .union([z.literal("pnpm"), z.literal("npm"), z.literal("yarn")])
  .catch("pnpm");
...
export function PackageManagerCommand({ content }: PackageManagerCommandProps) {
  ...
  useEffect(() => {
    const onPackageManagerChange = () => {
      ...
      const newValue = packageManagerSchema.parse(
        window.localStorage.getItem("packageManager")
      );
